...
import CerrarCampeonatoYAsignarPuntos from "../usecases/Campeonatos/CerrarCampeonatoYAsignarPuntos.js";
...
class CampeonatosController {
  ...
  async getCampeonatoById(req, res) {
    try {
      ...
      if (!id) {
        return res
          .status(401)
          .json({ error: "No se encontró el ID del campeonato" });
      }
  ...
  async cerrarYAsignar(req, res) {
    try {
      ...
      const id = req.params.id;
      if (!id)
        return res.status(400).json({ error: "ID de campeonato requerido" });

      const out = await new CerrarCampeonatoYAsignarPuntos().execute(id);
      return res.json(out);
    } catch (e) {
      console.error("Error cerrando y asignando puntos:", e);
