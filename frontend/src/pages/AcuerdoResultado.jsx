...
export default function AcuerdoResultado() {
  ...
  const yaFinalizado = partido?.estado === "finalizado" || Array.isArray(partido?.ganadores) && partido.ganadores.length > 0;
  ...
  const ganadorLabels = (ids) => {
    const map = new Map(federados.map(f => [f.id, `${f.nombre ?? ""} ${f.apellido ?? ""}`.trim() || f.email || f.id]));
  ...
  const onProponer = async () => {
    ...
    if (!resultado.trim())
      return alert("Ingresa el resultado, ej: 6-3 6-4");

    if (!Array.isArray(ganadores) || ganadores.length === 0 || ganadores.length > maxWinners)
      return alert(`Selecciona ${maxWinners === 1 ? "1" : `hasta ${maxWinners}`} ganador(es).`);
  ...
  const opcionesGanadores = useMemo(
    () => jugadoresDetalle.filter((p) =>
      `${p.nombre ?? ""} ${p.apellido ?? ""}`.toLowerCase().includes(pickerQuery.toLowerCase())
    ),
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="max-w-xl mx-auto px-4" style={{ paddingTop: "6rem" }}>
          ...
          <button className="mt-4 px-4 py-2 rounded bg-neutral-800 text-white" onClick={() => navigate(-1)}>Volver</button>
        </div>
      </div>
    );
  }

  const readOnly = !soyJugador || yaFinalizado || partido?.estadoResultado === "en_disputa";
  const tituloEstado = yaFinalizado
    ? "Finalizado"
    : partido?.estadoResultado === "en_disputa"
      ? "En disputa (admin resolverá)"
      : propuestaPendiente
        ? yoPropuse ? "Propuesta enviada — esperando confirmación" : "Tienes una propuesta para confirmar"
        : "Pendiente de propuesta";

  return (
    <div className="min-h-screen">
      <Navbar />
      <div className="mx-auto max-w-3xl px-4" style={{ paddingTop: "6rem", paddingBottom: "3rem" }}>
        <h1 className="text-3xl font-extrabold text-white">Acuerdo de resultado</h1>
        <div className="mt-2 text-white/80 text-sm">
          <div>Partido: <strong>{formatDT(partido.timestamp)}</strong> · {partido.tipoPartido} · Etapa: {partido.etapa || "—"}</div>
          <div>Estado: <span className="font-semibold">{tituloEstado}</span></div>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-white/15 bg-neutral-900/70 p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {jugadoresDetalle.map((j) => (
              <span key={j.id} className="px-2 py-1 rounded-full text-xs border border-white/20 text-white/90 bg-white/10">
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-4">
          <div className="rounded-xl border border-white/15 bg-neutral-900/70 p-4">
            ...
            />
            {!!partido.resultado && (
              <div className="mt-2 text-xs text-white/60">Resultado actual: {partido.resultado}</div>
            )}
          </div>
          ...
          <div className="rounded-xl border border-white/15 bg-neutral-900/70 p-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                onClick={abrirPicker}
                className={`px-3 py-1.5 rounded-lg border ${readOnly ? "border-white/20 text-white/40" : "border-white/30 text-white hover:bg-white/10"}`}
              >
                Seleccionar
              </button>
            </div>
            <div className="mt-2 text-white/90 text-sm">
              {ganadores.length ? ganadorLabels(ganadores) : <span className="text-white/50">Ninguno</span>}
            </div>
            {!!partido.ganadores?.length && (
              <div className="mt-2 text-xs text-white/60">Ganadores actuales: {ganadorLabels(partido.ganadores)}</div>
            )}
          </div>
        </div>
      ...
      {pickerOpen && (
        <Modal onClose={cerrarPicker}>
          <div className="p-5">
            <div className="flex items-center justify-between">
              <h3 className="text-white text-lg font-semibold">Seleccionar ganador(es) (máx {maxWinners})</h3>
              <button onClick={cerrarPicker} className="px-3 py-1.5 rounded-lg border border-white/30 text-white hover:bg-white/10">Cerrar</button>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {opcionesGanadores.map((j) => {
                const id = j.id;
                const label = `${j.nombre ?? ""} ${j.apellido ?? ""}`.trim() || j.email || id;
                const active = ganadores.includes(id);
                return (
                  <button
                    key={id}
                    onClick={() => toggleWinner(id)}
                    className={`px-3 py-1.5 rounded-full text-xs border ${active ? "bg-cyan-700 text-white border-cyan-500" : "bg-neutral-800 text-white/90 border-white/20 hover:bg-neutral-700"}`}
                  >
                    {label}
                  </button>
                );
            ...
            <div className="mt-5 flex justify-end">
              <button onClick={cerrarPicker} className="px-4 py-2 rounded-xl bg-cyan-700 text-white hover:bg-cyan-600">Listo</button>
            </div>
          </div>
        </Modal>
      )}
...
function Modal({ children, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={onClose}>
      <div className="max-w-xl w-full rounded-2xl bg-neutral-900 text-white border border-white/20 shadow-xl" onClick={(e) => e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
